...
```python
try:
    result = await client.logistic.getPincodeCity(pincode=pincode, xApplicationId=xApplicationId)
    # use result
...
```python
try:
    result = await client.logistic.getTatProduct(xApplicationId=xApplicationId, body=body)
    # use result
except Exception as e:
    print(e)
```
