...
```python
try:
    result = await client.logistic.getPincodeView(pincode=pincode, xApplicationId=xApplicationId)
    # use result
...
```python
try:
    result = await client.logistic.getTATView(xApplicationId=xApplicationId, body=body)
    # use result
except Exception as e:
    print(e)
```
