...
```python
try:
    result = await applicationClient.logistic.getCountries(onboarding=onboarding)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await applicationClient.logistic.getLocalities(localityType=localityType, country=country, state=state, city=city)
    # use result
except Exception as e:
    print(e)
```
