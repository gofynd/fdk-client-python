...
```python
try:
    result = await client.cart.getCart(id=id, i=i, b=b, assignCardId=assignCardId, areaCode=areaCode, emptyCart=emptyCart)
    # use result
...
```python
try:
    result = await client.cart.getShipments(p=p, id=id, addressId=addressId, areaCode=areaCode, orderType=orderType)
    # use result
except Exception as e:
    print(e)
```
