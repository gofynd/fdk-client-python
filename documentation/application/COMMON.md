...
```python
try:
    result = await applicationClient.common.searchApplication(authorization=authorization, query=query)
    # use result
...
```python
try:
    result = await applicationClient.common.getLocations(locationType=locationType, id=id)
    # use result
except Exception as e:
    print(e)
```
