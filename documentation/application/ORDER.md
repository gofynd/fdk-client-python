...
```python
try:
    result = await applicationClient.order.getInvoiceByShipmentId(shipmentId=shipmentId, documentType=documentType)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await applicationClient.order.getShipmentBagReasons(shipmentId=shipmentId, bagId=bagId)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await applicationClient.order.getProducts(status=status, pageNo=pageNo, pageSize=pageSize, fromDate=fromDate, toDate=toDate, searchValue=searchValue)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await applicationClient.order.updateShipmentStatus1(shipmentId=shipmentId, body=body)
    # use result
except Exception as e:
    print(e)
```
...
| body | [UpdateShipmentStatusRequest1](#UpdateShipmentStatusRequest1) | yes | Request body |
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | filters | ArrayList<[ProductsDataUpdatesFilters1](#ProductsDataUpdatesFilters1)>? |  yes  | Filter for the Product/Bag |
 | data | HashMap<String,Any>? |  yes  |  |
...
---
 ...
 | products | ArrayList<[Products1](#Products1)>? |  yes  | Product/Bag to be updated |
...
---
 ...
 #### [UpdateShipmentStatusRequest1](#UpdateShipmentStatusRequest1)
