...
```python
try:
    result = await client.order.getReasons(shipmentId=shipmentId, bagId=bagId)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.order.verifyOtp(orderId=orderId, shipmentId=shipmentId, body=body)
    # use result
except Exception as e:
    print(e)
```
