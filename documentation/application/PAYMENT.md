...
```python
try:
    result = await client.payment.getPaymentLink(id=id)
    # use result
except Exception as e:
    print(e)
```
...
```json
{
  ...
  "payment_link_url": "https://api.fyndx0.de/service/application/payment/v1.0/payment/options/link/?id=4adb4451720a82ee69a3c1c4cc9ab442",
...
```json
{
  ...
  "status_code": 200,
  "payment_link_url": "https://api.fyndx0.de/service/application/payment/v1.0/payment/options/link/?id=96b15ea2014a76c8d2774783e239ce26",
...
[CancelOrResendPaymentLinkResponse](#CancelOrResendPaymentLinkResponse)
...
```python
try:
    result = await client.payment.getPaymentModeRoutesPaymentLink(id=id)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.payment.pollingPaymentLink(id=id)
    # use result
except Exception as e:
    print(e)
```
...
```json
{
  "aggregator_name": "Fynd",
  "status": "paid",
  "redirect_url": "https://api.jiox0.de/platform/payment/v2/external/payments/callback/?success=false&order_id=FY6273C56E012DFA0A00&aggregator_name=Fynd&cart_id=412&delivery_address_id=136&billing_address_id=136",
  "success": true,
  "http_status": 200
}
```
...
```python
try:
    result = await client.payment.redirectToAggregator(source=source)
    # use result
except Exception as e:
    print(e)
```
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | error | HashMap<String,Any> |  no  | error details |
 | success | Boolean |  no  | Response is successful or not |
...
---
 ...
 | kycUrl | String? |  yes  | Epaylater KYC banner url. |
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | paymentLinkCurrentStatus | String |  no  | Status of payment link |
 | paymentLinkUrl | String |  no  | Url of payment link |
...
---
 ...
 | meta | HashMap<String,Any> |  no  | Meta |
...
---
 ...
 | ---------- | ---- | -------- | ----------- |
 | success | Boolean |  no  | Successful or failure |
 | aggregatorName | String |  no  | Aggregator name |
 | redirectUrl | String |  no  | Url to redirect to |
 | status | String |  no  | Status of payment link |
 | httpStatus | Int |  no  | HTTP status code |
...
---
 ...
 | balance | ArrayList<[BalanceDetails](#BalanceDetails)>? |  yes  | Credit summary of user. |
...
---
 ...
 | data | ArrayList<[CreditSummary](#CreditSummary)>? |  yes  | Credit summary of user. |
...
---
 ...
 | data | ArrayList<[RedirectURL](#RedirectURL)> |  no  | Redirect URL. |
...
---
 ...
 | data | ArrayList<[CreditDetail](#CreditDetail)> |  no  | Credit summary of user. |
...
---
 ...
 | address | ArrayList<[KYCAddress](#KYCAddress)>? |  yes  | Address details |
 | pan | String? |  yes  | Pan Number |
...
---
 ...
 | ---------- | ---- | -------- | ----------- |
 | success | Boolean |  no  | Status updated or not. |
 | data | ArrayList<[OnboardSummary](#OnboardSummary)> |  no  | Redirect URL. |
