...
```python
try:
    result = await applicationClient.poscart.getCart(id=id, i=i, b=b, p=p, assignCardId=assignCardId, areaCode=areaCode, buyNow=buyNow)
    # use result
...
```python
try:
    result = await applicationClient.poscart.addItems(i=i, b=b, p=p, areaCode=areaCode, buyNow=buyNow, id=id, body=body)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await applicationClient.poscart.updateCart(id=id, i=i, b=b, p=p, areaCode=areaCode, buyNow=buyNow, body=body)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await applicationClient.poscart.validateCouponForPayment(id=id, buyNow=buyNow, addressId=addressId, paymentMode=paymentMode, paymentIdentifier=paymentIdentifier, aggregatorName=aggregatorName, merchantCode=merchantCode, iin=iin, network=network, type=type, cardId=cardId)
    # use result
except Exception as e:
    print(e)
```
