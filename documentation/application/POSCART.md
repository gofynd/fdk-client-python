...
```python
try:
    result = await client.poscart.getAddresses(cartId=cartId, buyNow=buyNow, mobileNo=mobileNo, checkoutMode=checkoutMode, tags=tags, isDefault=isDefault, userId=userId)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.poscart.removeAddress(id=id, userId=userId)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.poscart.selectAddress(cartId=cartId, buyNow=buyNow, i=i, b=b, userId=userId, body=body)
    # use result
except Exception as e:
    print(e)
```
