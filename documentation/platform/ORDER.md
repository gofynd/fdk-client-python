...
```python
try:
    result = await platformClient.order.generateInvoiceID(invoiceType=invoiceType, body=body)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await platformClient.order.getShipmentById(channelShipmentId=channelShipmentId, shipmentId=shipmentId, fetchActiveShipment=fetchActiveShipment, allowInactive=allowInactive)
    # use result
except Exception as e:
    print(e)
```
