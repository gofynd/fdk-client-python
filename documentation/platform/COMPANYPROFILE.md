...
```python
try:
    result = await client.companyprofile.getLocations(storeType=storeType, q=q, stage=stage, storeCode=storeCode, pageNo=pageNo, pageSize=pageSize)
    # use result
except Exception as e:
    print(e)
```
...
| storeCode | ArrayList<Any>? | no | to filter stores on basis of store code. |   
| pageNo | Int? | no | The page number to navigate through the given set of results |   
| pageSize | Int? | no | Number of items to retrieve in each page. Default is 10. |  
...
---
 ...
 | taxes | ArrayList<[CompanyTax](#CompanyTax)>? |  yes  |  |
...
---
 ...
 | businessDetails | [BusinessDetails](#BusinessDetails)? |  yes  |  |
 | taxes | ArrayList<HashMap<String,Any>>? |  yes  |  |
