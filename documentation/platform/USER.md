...
```python
try:
    result = await client.application("<APPLICATION_ID>").user.deleteSession(id=id)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.application("<APPLICATION_ID>").user.getUserGroups(pageNo=pageNo, pageSize=pageSize, name=name, status=status)
    # use result
except Exception as e:
    print(e)
```
