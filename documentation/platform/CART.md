...
```python
try:
    result = await client.application("<APPLICATION_ID>").cart.addItems(i=i, b=b, buyNow=buyNow, id=id, body=body)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.application("<APPLICATION_ID>").cart.deleteCart(id=id)
    # use result
except Exception as e:
    print(e)
```
