...
```python
try:
    result = await platformClient.application("<APPLICATION_ID>").catalog.getProductSizeSellers(itemId=itemId, size=size, pincode=pincode, strategy=strategy, pageNo=pageNo, pageSize=pageSize)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await platformClient.catalog.deleteRealtimeInventory(itemId=itemId, sellerIdentifier=sellerIdentifier)
    # use result
except Exception as e:
    print(e)
```
