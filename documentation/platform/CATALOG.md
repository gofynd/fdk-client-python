...
```python
try:
    result = await client.application("<APPLICATION_ID>").catalog.upsertAppProductMeta(itemId=itemId, body=body)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.application("<APPLICATION_ID>").catalog.getApplicationBrands(department=department, pageNo=pageNo, pageSize=pageSize, q=q, uid=uid)
    # use result
except Exception as e:
    print(e)
```
