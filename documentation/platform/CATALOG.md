...
```python
try:
    result = await platformClient.application("<APPLICATION_ID>").catalog.getAppProducts(brandIds=brandIds, categoryIds=categoryIds, departmentIds=departmentIds, tags=tags, isDependent=isDependent, pageNo=pageNo, pageSize=pageSize, q=q)
    # use result
except Exception as e:
    print(e)
```
