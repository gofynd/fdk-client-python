...
```python
try:
    result = await client.catalog.getProductExportJobs(status=status, fromDate=fromDate, toDate=toDate, q=q)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.application("<APPLICATION_ID>").catalog.getAppProducts(brandIds=brandIds, categoryIds=categoryIds, departmentIds=departmentIds, tags=tags, isDependent=isDependent, itemIds=itemIds, pageNo=pageNo, pageSize=pageSize, q=q)
    # use result
except Exception as e:
    print(e)
```
