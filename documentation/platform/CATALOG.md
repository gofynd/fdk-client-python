...
```python
try:
    result = await client.catalog.listProductTemplate(department=department, pageNo=pageNo, pageSize=pageSize)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.application("<APPLICATION_ID>").catalog.getAppInventoryStatus(itemId=itemId, storeIds=storeIds, brandIds=brandIds, sellerIdentifier=sellerIdentifier, timestamp=timestamp, pageSize=pageSize, pageId=pageId)
    # use result
except Exception as e:
    print(e)
```
