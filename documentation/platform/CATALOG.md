...
```python
try:
    result = await client.application("<APPLICATION_ID>").catalog.getAllCollections(q=q, scheduleStatus=scheduleStatus, tags=tags, isActive=isActive, pageNo=pageNo, pageSize=pageSize)
    # use result
except Exception as e:
    print(e)
```
