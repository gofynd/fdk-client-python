...
```python
try:
    result = await client.orders.getLaneConfig(superLane=superLane, fromDate=fromDate, toDate=toDate)
    # use result
except Exception as e:
    print(e)
```
...
```python
try:
    result = await client.orders.getShipmentList(lane=lane, searchType=searchType, searchId=searchId, fromDate=fromDate, toDate=toDate)
    # use result
except Exception as e:
    print(e)
```
