...
class Rewards:
    ...
    async def getPoints(self, body=""):
        ...
        try:
            ...
            print("Response Validation failed for getPoints")
            print(e)
    ...
    async def referral(self, body=""):
        ...
        try:
            ...
            print("Response Validation failed for referral")
            print(e)
    ...
    async def orderDiscount(self, body=""):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        response = await AiohttpHelper().aiohttp_request("POST", url_with_params, headers=get_headers_with_signature(urlparse(self._urls["orderDiscount"]).netloc, "post", await create_url_without_domain("/service/application/rewards/v1.0/user/offer/order-discount/", ), query_string, headers, body, exclude_headers=exclude_headers), data=body, cookies=self._conf.cookies)
        ...
        try:
            ...
            print("Response Validation failed for orderDiscount")
            print(e)
