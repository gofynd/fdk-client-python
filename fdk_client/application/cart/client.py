...
class Cart:
    ...
    async def getPromotions(self, page_size=None, page_no=None, promotion_type=None, is_bank_offer=None, body="", request_headers:Dict={}):
        ...
        url_with_params = await create_url_with_params(api_url=self._urls["getPromotions"], proccessed_params="""{"required":[],"optional":[{"name":"page_size","description":"Number of offers to be fetched to show.","in":"query","schema":{"type":"integer"}},{"name":"page_no","description":"Page number to be fetched to show","in":"query","schema":{"type":"integer"}},{"name":"promotion_type","description":"Promotion type of promotions to be fetched","in":"query","schema":{"type":"string","x-not-enum":true}},{"name":"is_bank_offer","description":"Boolean flag stating if promotions with bank offer need to be fetched.","in":"query","schema":{"type":"boolean"}}],"query":[{"name":"page_size","description":"Number of offers to be fetched to show.","in":"query","schema":{"type":"integer"}},{"name":"page_no","description":"Page number to be fetched to show","in":"query","schema":{"type":"integer"}},{"name":"promotion_type","description":"Promotion type of promotions to be fetched","in":"query","schema":{"type":"string","x-not-enum":true}},{"name":"is_bank_offer","description":"Boolean flag stating if promotions with bank offer need to be fetched.","in":"query","schema":{"type":"boolean"}}],"headers":[],"path":[]}""", serverType="application", page_size=page_size, page_no=page_no, promotion_type=promotion_type, is_bank_offer=is_bank_offer)
