...
class OAuthClient:
    ...
    def startAuthorization(self, options: Dict):
        ...
        reqPath = f"/service/panel/authentication/v1.0/organization/{self.config.organizationId}/oauth/authorize"
        signingOptions = {
          "method": "GET",
        ...
        queryString = get_headers_with_signature(self.config.domain, "get",
                                                            f"/service/panel/authentication/v1.0/organization/"
                                                            f"{self.config.organizationId}/oauth/authorize",
    ...
    async def renewAccessToken(self, is_offline_token=False):
        if is_offline_token:
            request_cache_key = f"{self.config.apiKey}:{self.config.organizationId}"
    ...
    async def getAccesstokenObj(self, grant_type="", refresh_token="", code=""):
        ...
        url = f"{self.config.domain}/service/panel/authentication/v1.0/organization/{self.config.organizationId}/oauth/token"
        headers = {
            "Authorization": f"Basic {token}"
        }
        headers = get_headers_with_signature(self.config.domain, "post",
                                             f"/service/panel/authentication/v1.0/organization/{self.config.organizationId}/oauth/token",
    ...
    async def getOfflineAccessTokenObj(self, scopes, code):
        url = f"{self.config.domain}/service/panel/authentication/v1.0/organization/{self.config.organizationId}/oauth/offline-token"
        data = {
            "client_id": self.config.apiKey,
        ...
        headers = get_headers_with_signature(
            ...
            method="post",
            url=f"/service/panel/authentication/v1.0/organization/{self.config.organizationId}/oauth/offline-token",
            query_string="",
            headers=headers,
            body=data,
