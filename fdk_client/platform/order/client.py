...
class Order:
    ...
    async def getShipmentTracking(self, shipment_id=None):
        """
        :param shipment_id :  : type string
        """
        payload = {}
        ...
        # Parameter validation
        schema = OrderValidator.getShipmentTracking()
        schema.dump(schema.load(payload))
        

        url_with_params = await create_url_with_params(self._conf.domain, f"/service/platform/dp/v1.0/company/{self._conf.companyId}/tracking", """{"required":[{"in":"query","name":"shipment_id","required":true,"schema":{"type":"string"}},{"in":"path","name":"company_id","required":true,"schema":{"type":"integer"}}],"optional":[],"query":[{"in":"query","name":"shipment_id","required":true,"schema":{"type":"string"}}],"headers":[],"path":[{"in":"path","name":"company_id","required":true,"schema":{"type":"integer"}}]}""", shipment_id=shipment_id, )
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/dp/v1.0/company/{self._conf.companyId}/tracking", shipment_id=shipment_id, ), query_string, headers, "", exclude_headers=exclude_headers), data="")
