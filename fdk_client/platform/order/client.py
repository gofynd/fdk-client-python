...
The ESM config stores order processing configuration. Each document in the ESM config collection of  Order Management System - OMS's database is a JSON object representing the configuration of a specific application ID. This includes filters, hooks, flags set on different state-transitions.  This configuration is picked and accordingly features are enabled. 
    ...
    async def createOrder(self, x_ordering_source=None, x_application_id=None, x_extension_id=None, body="", request_headers:Dict={}):
        """Creates an order in the OMS. Note: Use the Serviceability API (getShipments) to determine shipments before creating an order. OMS no longer auto-selects fulfillment stores and only creates shipments as provided in the request payload.
        :param x-ordering-source : To uniquely identify the source through which order has been placed. : type string
        :param x-application-id : application id : type string
        :param x-extension-id : extension id : type string
        """
        payload = {}
        ...
        url_with_params = await create_url_with_params(self._conf.domain, f"/service/platform/order-manage/v1.0/company/{self._conf.companyId}/orders", """{"required":[{"name":"company_id","in":"path","description":"","required":true,"schema":{"type":"integer"}},{"in":"header","name":"x-ordering-source","description":"To uniquely identify the source through which order has been placed.","required":true,"schema":{"type":"string"}}],"optional":[{"name":"x-application-id","in":"header","description":"application id","required":false,"example":"66fe99245f70599b2d63d312","schema":{"type":"string"}},{"name":"x-extension-id","in":"header","description":"extension id","required":false,"example":"66fe99245f70599b2d63d312","schema":{"type":"string"}}],"query":[],"headers":[{"in":"header","name":"x-ordering-source","description":"To uniquely identify the source through which order has been placed.","required":true,"schema":{"type":"string"}},{"name":"x-application-id","in":"header","description":"application id","required":false,"example":"66fe99245f70599b2d63d312","schema":{"type":"string"}},{"name":"x-extension-id","in":"header","description":"extension id","required":false,"example":"66fe99245f70599b2d63d312","schema":{"type":"string"}}],"path":[{"name":"company_id","in":"path","description":"","required":true,"schema":{"type":"integer"}}]}""", serverType="platform", x_ordering_source=x_ordering_source, x_application_id=x_application_id, x_extension_id=x_extension_id)
        query_string = await create_query_string()
        if query_string:
            url_with_params += "?" + query_string
