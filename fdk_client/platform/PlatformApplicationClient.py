...
class Payment:
    ...
    async def repaymentDetails(self, body=""):
        ...
        # Body validation
        from .models.RepaymentRequestDetails import RepaymentRequestDetails
        schema = RepaymentRequestDetails()
        schema.dump(schema.load(body))
...
class Cart:
    ...
    async def getPromosCouponConfig(self, ):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/cart/v1.0/company/{self._conf.companyId}/application/{self.applicationId}/active-promotion", ), query_string, headers, "", exclude_headers=exclude_headers), data="")
