...
class Configuration:
    ...
    async def getDomains(self, comapny_id=None, appication_id=None):
        """Gives the domains list attached with the current application.

**Note**
* Gets Maximum 4 domains      

        :param comapny_id : Current company id : type string
        :param appication_id : current application id : type string
        """
        payload = {}
        
        if comapny_id:
            payload["comapny_id"] = comapny_id
        
        if appication_id:
            payload["appication_id"] = appication_id
        ...
        url_with_params = await create_url_with_params(self._conf.domain, f"/service/platform/configuration/v1.0/company/{self._conf.companyId}/application/{self.applicationId}/domain", """{"required":[{"schema":{"type":"string"},"description":"Current company id","in":"path","required":true,"name":"company_id"},{"schema":{"type":"string"},"description":"Current application id","in":"path","required":true,"name":"application_id"},{"name":"comapny_id","in":"path","required":true,"description":"Current company id","schema":{"type":"string"}},{"name":"appication_id","in":"path","required":true,"description":"current application id","schema":{"type":"string"}}],"optional":[],"query":[],"headers":[],"path":[{"schema":{"type":"string"},"description":"Current company id","in":"path","required":true,"name":"company_id"},{"schema":{"type":"string"},"description":"Current application id","in":"path","required":true,"name":"application_id"},{"name":"comapny_id","in":"path","required":true,"description":"Current company id","schema":{"type":"string"}},{"name":"appication_id","in":"path","required":true,"description":"current application id","schema":{"type":"string"}}]}""", comapny_id=comapny_id, appication_id=appication_id)
        query_string = await create_query_string(comapny_id=comapny_id, appication_id=appication_id)
        headers = {
            "Authorization": "Bearer " + await self._conf.getAccessToken()
        }
        for h in self._conf.extraHeaders:
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/configuration/v1.0/company/{self._conf.companyId}/application/{self.applicationId}/domain", comapny_id=comapny_id, appication_id=appication_id), query_string, headers, "", exclude_headers=exclude_headers), data="")
