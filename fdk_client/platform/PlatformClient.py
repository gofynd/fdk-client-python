...
class Order:
    ...
    async def updateShipmentStatus(self, body=""):
        ...
        # Body validation
        from .models.UpdateShipmentStatusPayload import UpdateShipmentStatusPayload
        schema = UpdateShipmentStatusPayload()
        schema.dump(schema.load(body))
        

        url_with_params = await create_url_with_params(self._conf.domain, f"/service/platform/order-manage/v1.0/company/{self._conf.companyId}/update-shipment-status", """{"required":[{"in":"path","name":"company_id","required":true,"schema":{"type":"integer"}}],"optional":[],"query":[],"headers":[],"path":[{"in":"path","name":"company_id","required":true,"schema":{"type":"integer"}}]}""", )
        query_string = await create_query_string()
        headers = {
            "Authorization": "Bearer " + await self._conf.getAccessToken()
        }
    ...
    async def getOrderById(self, fynd_order_id=None, page_no=None, page_size=None):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/order/v1.0/company/{self._conf.companyId}/order-details", fynd_order_id=fynd_order_id, page_no=page_no, page_size=page_size), query_string, headers, "", exclude_headers=exclude_headers), data="")
    ...
    async def getShipmentByAffiliateBagId(self, affiliate_bag_id=None, affiliate_id=None, page_no=None, page_size=None):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/order/v1.0/company/{self._conf.companyId}/shipment-details-by-affiliate-bag-id", affiliate_bag_id=affiliate_bag_id, affiliate_id=affiliate_id, page_no=page_no, page_size=page_size), query_string, headers, "", exclude_headers=exclude_headers), data="")
    ...
    async def getShipmentByAffiliateShipmentId(self, affiliate_shipment_id=None, affiliate_id=None):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/order/v1.0/company/{self._conf.companyId}/shipment-details-by-affiliate-shipment-id", affiliate_shipment_id=affiliate_shipment_id, affiliate_id=affiliate_id), query_string, headers, "", exclude_headers=exclude_headers), data="")
    ...
    async def getShipmentsByAffiliateOrderId(self, affiliate_id=None, affiliate_order_id=None, page_no=None, page_size=None):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/order/v1.0/company/{self._conf.companyId}/shipment-details-by-affiliate-order-id", affiliate_id=affiliate_id, affiliate_order_id=affiliate_order_id, page_no=page_no, page_size=page_size), query_string, headers, "", exclude_headers=exclude_headers), data="")
