...
class Order:
    ...
    async def statusUpdateInternalV4(self, body=""):
        ...
        # Parameter validation
        schema = OrderValidator.statusUpdateInternalV4()
        schema.dump(schema.load(payload))
...
class DocumentEngine:
    ...
    async def getBulkShipmentStatus(self, batch_id=None):
        """Use this API to fetch status of PDF generation of Bulk Invoice and Label
        :param batch_id : batch_id given at time of generate request : type string
        """
        payload = {}
        ...
        # Parameter validation
        schema = DocumentEngineValidator.getBulkShipmentStatus()
        schema.dump(schema.load(payload))
        

        url_with_params = await create_url_with_params(self._conf.domain, f"/service/platform/document/v2.0/company/{self._conf.companyId}/bulk-shipment-status", """{"required":[{"in":"path","name":"company_id","required":true,"description":"Company ID","schema":{"type":"integer"}},{"in":"query","name":"batch_id","required":true,"description":"batch_id given at time of generate request","schema":{"type":"string"}}],"optional":[],"query":[{"in":"query","name":"batch_id","required":true,"description":"batch_id given at time of generate request","schema":{"type":"string"}}],"headers":[],"path":[{"in":"path","name":"company_id","required":true,"description":"Company ID","schema":{"type":"integer"}}]}""", batch_id=batch_id)
        query_string = await create_query_string(batch_id=batch_id)
        headers = {
            "Authorization": "Bearer " + await self._conf.getAccessToken()
        }
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=await get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/document/v2.0/company/{self._conf.companyId}/bulk-shipment-status", batch_id=batch_id), query_string, headers, "", exclude_headers=exclude_headers), data="")
