...
class DocumentEngine:
    ...
    async def generateNoc(self, body=""):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("POST", url_with_params, headers=get_headers_with_signature(self._conf.domain, "post", await create_url_without_domain(f"/service/platform/document/v1.0/company/{self._conf.companyId}/generate-noc", ), query_string, headers, body, exclude_headers=exclude_headers), data=body)
    ...
    async def getLabelStatus(self, uid=None):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/document/v1.0/company/{self._conf.companyId}/get-label-list", uid=uid), query_string, headers, "", exclude_headers=exclude_headers), data="")
    ...
    async def getNocStatus(self, uid=None):
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("GET", url_with_params, headers=get_headers_with_signature(self._conf.domain, "get", await create_url_without_domain(f"/service/platform/document/v1.0/company/{self._conf.companyId}/get-noc-status", uid=uid), query_string, headers, "", exclude_headers=exclude_headers), data="")
    ...
    async def generatePOSReceipts(self, body=""):
        ...
        # Parameter validation
        schema = DocumentEngineValidator.generatePOSReceipts()
        schema.dump(schema.load(payload))
        
        # Body validation
        from .models import GeneratePOSReceipts
        schema = GeneratePOSReceipts()
        schema.dump(schema.load(body))
        

        url_with_params = await create_url_with_params(self._conf.domain, f"/service/platform/document/v2.0/company/{self._conf.companyId}/pos/generate-receipt", """{"required":[{"in":"path","name":"company_id","required":true,"description":"Company ID","schema":{"type":"integer"}}],"optional":[],"query":[],"headers":[],"path":[{"in":"path","name":"company_id","required":true,"description":"Company ID","schema":{"type":"integer"}}]}""", )
        query_string = await create_query_string()
        headers = {
            "Authorization": "Bearer " + await self._conf.getAccessToken()
        }
        ...
        for key, val in headers.items():
            if not key.startswith("x-fp-"):
                exclude_headers.append(key)
        return await AiohttpHelper().aiohttp_request("POST", url_with_params, headers=get_headers_with_signature(self._conf.domain, "post", await create_url_without_domain(f"/service/platform/document/v2.0/company/{self._conf.companyId}/pos/generate-receipt", ), query_string, headers, body, exclude_headers=exclude_headers), data=body)
